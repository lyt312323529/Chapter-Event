...
for i in range(len(postags)):
    ...
    if pt=="v":
        ...
        print("词语(" + words[i] + ")的标签为(" + postags[i] + "),符合词性要求，放入标题事件元素列表")
        title_argument.append(words[i])
    else:
    ...
    if (relations_list[i][0][0]!="is")and(relations_list[i][0][0]!="de"):
        ...
        if len(trigger)>0:
            ...
            if len(arguments_item)>0:
                ...
                print("事件元素列表不为空，将事件("+str(len(events_list)-1)+")放入事件列表中,该事件对应的句子id为:"+str(relations_list[i][2]))
            else:
...
for i in range(len(events_list)):
    for j in range(len(events_list[i][0])):
        ...
        if word_exist_state:
            print("事件("+str(i)+")中事件触发词("+events_list[i][0][j]+")已经存在于单词列表中,不重复加入,对应的句子id为:("+str(events_list[i][2])+")")
        else:
            word_sentence_id=[]
            words_list.append(events_list[i][0][j])
            word_sentence_id.append(events_list[i][2])
            words_sentence_id_list.append(word_sentence_id)
            print("\n事件("+str(i)+")中将事件触发词("+events_list[i][0][j]+")加入单词列表,对应的句子id为:("+str(events_list[i][2])+")")
    ...
    for l in range(len(events_list[i][1])):
        ...
        for m in range(len(words_list)):
            if words_list[m]==events_list[i][1][l]:
                ...
                word_exist_state=True
        if word_exist_state:
            print("事件("+str(i)+")中事件元素("+events_list[i][1][l]+")已经存在于单词列表中,不重复加入,对应的句子id为:("+str(events_list[i][2])+")")
        else:
            word_sentence_id=[]
            word_sentence_id.append(events_list[i][2])
            words_list.append(events_list[i][1][l])
            words_sentence_id_list.append(word_sentence_id)
            print("\n事件("+str(i)+")中将事件元素("+events_list[i][1][l]+")加入单词列表,对应的句子id为:("+str(events_list[i][2])+")")
...
for i in range(len(events_list)):
    ...
    if len(events_list[i][0])>1:
        for j in range(len(events_list[i][0])-1):
            ...
            if (not dg.has_edge((relation1_index, relation2_index))):
                ...
                print("事件("+str(i)+")中的关系词("+str(events_list[i][0][j])+")与关系词("+str(events_list[i][0][j+1])+")建立一条边")
            else:
                print("事件("+str(i)+")中的关系词("+str(events_list[i][0][j])+")与关系词("+str(events_list[i][0][j+1])+")已经存在一条边，不需要重复建立")
    else:
    ...
    if len(events_list[i][1])>1:
        for j in range(len(events_list[i][1])-1):
            ...
            if (not dg.has_edge((relation1_index, relation2_index))):
                dg.add_edge((relation1_index, relation2_index))
                print("事件("+str(i)+")中的事件元素("+str(events_list[i][1][j])+")与事件元素("+str(events_list[i][1][j+1])+")建立一条边")
            else:
                print("事件("+str(i)+")中的事件元素("+str(events_list[i][1][j])+")与事件元素("+str(events_list[i][1][j+1])+")已经存在一条边，不需要重复建立")
    else:
    ...
    for j in range(len(events_list[i][0])):
        for k in range(len(events_list[i][1])):
            ...
            if (not dg.has_edge((relation1_index, relation2_index))):
                dg.add_edge((relation1_index, relation2_index))
                print("事件("+str(i)+")中的关系词("+str(events_list[i][0][j])+")与事件元素("+str(events_list[i][1][k])+")建立一条边")
            else:
                print("事件("+str(i)+")中的关系词("+str(events_list[i][0][j])+")与事件元素("+str(events_list[i][1][k])+")已经存在一条边，不需要重复建立")
...
f=open("/media/lyt312323529/c4175817-9d97-490b-95c6-636149e75a87/Graph_Generate/news_"+str(news_index)+"_result.txt","w")
